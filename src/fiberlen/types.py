...
class NodeKind(str, Enum):
    ENDPOINT = "endpoint"
    JUNCTION = "junction"
    JUNCTION_ELEMENT = "junction_element"  # blobを構成していた要素ノード（統計対象外）


class SegmentKind(str, Enum):
    SEGMENT = "segment"
    JUNCTION_ELEMENT = "junction_element"  # blob内の短セグメント等（統計対象外）
...
class Node:
    ...
    kind: endpoint / junction / junction_element
...
class Segment:
    ...
    kind: SegmentKind = SegmentKind.SEGMENT  # ★追加（通常はSEGMENT）
    ...
    def from_dict(d: Dict[str, Any]) -> "Segment":
        return Segment(
            ...
            kind=SegmentKind(str(d.get("kind", SegmentKind.SEGMENT.value))),  # ★追加（後方互換）
        )
