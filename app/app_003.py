...
um_per_px = float(st.sidebar.number_input("**um_per_px:**  \n setting scale here", value=float(CFG.um_per_px), step=0.1, format="%.1f"))

blur_sigma_px = float(st.sidebar.number_input("**blur_sigma_px:**  \n extent of blurring in generating background", value=float(CFG.blur_sigma_px), step=0.5, format="%.1f"))
...
if uploaded is None:
    ...
    st.sidebar.write("threshold_otsu: -")

    st.sidebar.markdown("---")
    st.sidebar.button("Run analysis  \n (skeletonize -> results)", disabled=True)
    st.sidebar.markdown("---")
    st.sidebar.header("Parameters (skeletonize -> results)")

    st.sidebar.number_input("**eliminate_length_px:**  \n elminate object with this px*px area before skeletonize", value=int(CFG.eliminate_length_px), step=1, disabled=True)
    st.sidebar.number_input("**border_margin_px:**  \n ignore object this px close to the edge", value=int(CFG.border_margin_px), step=1, disabled=True)
    st.sidebar.number_input("**merge_short_seg_px:**  \n merge nodes this px close to each other", value=int(CFG.merge_short_seg_px), step=1, disabled=True)

    st.sidebar.number_input("**threshold_of_nonlinear:**  \n L/D threshold for kink_cut condidate, where L: fiber length and D: edge distance", 
                            value=float(CFG.threshold_of_nonlinear), step=0.05, format="%.2f", disabled=True)

    st.sidebar.number_input("**(arm_length_px):**  \n arms length to calcuate kink angle", value=int(CFG.blob_px), step=1, disabled=True)
    st.sidebar.number_input("**(cut_max):** \n max times of kink_cut applied", value=int(CFG.cut_max), step=1, disabled=True)
    st.sidebar.number_input("**cut_angle:**  \n cut object at a kink over this angle in degree", value=float(CFG.cut_angle), step=1.0, format="%.0f", disabled=True)

    st.sidebar.number_input("**pairing_angle_max:**  \n connect object joined below this angle in degree", value=float(CFG.pairing_angle_max), step=1.0, format="%.0f", disabled=True)
    st.sidebar.number_input("**(pairing_length_for_calc_angle_px):**  \n arm length to calculate angle", value=int(CFG.pairing_length_for_calc_angle), step=1, disabled=True)

    st.sidebar.number_input("**(top_cut_px):**  \n recude fiber length with this px at both end", value=int(CFG.top_cut), step=1, disabled=True)
    st.sidebar.number_input("**post_eliminate_length_px:**  \n remove fibers less than this length from statistics", value=float(CFG.post_eliminate_length_px), step=5.0, format="%.0f", disabled=True)

    st.sidebar.number_input("**hist_range_min_um:**", value=float(CFG.hist_range[0]), step=10.0, format="%.0f", disabled=True)
    st.sidebar.number_input("**hist_range_max_um:**", value=float(CFG.hist_range[1]), step=10.0, format="%.0f", disabled=True)
...
st.sidebar.write(f"threshold_otsu (recommended): {threshold_otsu:.3f}")
...
st.sidebar.markdown("---")
st.sidebar.header("Parameters (2nd step: skeletonize -> results)")
...
threshold_of_nonlinear = float(st.sidebar.number_input("threshold_of_nonlinear", value=float(CFG.threshold_of_nonlinear), step=0.01, format="%.2f"))
...
cut_angle = float(st.sidebar.number_input("cut_angle", value=float(CFG.cut_angle), step=0.5, format="%.2f"))
