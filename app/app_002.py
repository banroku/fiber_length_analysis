...
# if "upper_cache" not in st.session_state:
#     st.session_state.upper_cache = None
# if "lower_cache" not in st.session_state:
...
# ----------------------------
st.sidebar.header("Parameters (upper: preprocess -> binarize)")

um_per_px = float(st.sidebar.number_input("um_per_px", value=float(CFG.um_per_px), step=0.1, format="%.6f"))

background_is_dark = bool(
    st.sidebar.number_input(
        "background_is_dark (0 or 1)",
...
blur_sigma_px = float(st.sidebar.number_input("blur_sigma_px", value=float(CFG.blur_sigma_px), step=0.5, format="%.6f"))
...
st.subheader("Upper (preprocess -> binarize)")
u1, u2 = st.columns(2)
with u1:
...
st.subheader("Lower (skeletonize -> results)")
l1, l2 = st.columns(2)
with l1:
...
st.write("Histogram (x: um, y: % of total length) with cumulative %")
hist_ph = st.empty()
...
# ----------------------------
# If no file yet: keep placeholders and still show parameter UI
# ----------------------------
if uploaded is None:
    st.sidebar.write("threshold_otsu (recommended): -")
    st.sidebar.number_input(
        "threshold_manual (0..1)",
        value=float(CFG.threshold),
        step=0.001,
        min_value=0.0,
        max_value=1.0,
        format="%.6f",
    ...
    st.sidebar.markdown("---")
    st.sidebar.button("Run/Update (lower: skeletonize -> results)", disabled=True)
    st.sidebar.markdown("---")
    st.sidebar.header("Parameters (lower: skeletonize -> results)")

    st.sidebar.number_input("eliminate_length_px", value=int(CFG.eliminate_length_px), step=1, disabled=True)
    st.sidebar.number_input("border_margin_px", value=int(CFG.border_margin_px), step=1, disabled=True)
    st.sidebar.number_input("merge_short_seg_px", value=int(CFG.merge_short_seg_px), step=1, disabled=True)

    st.sidebar.number_input("threshold_of_nonlinear", value=float(CFG.threshold_of_nonlinear), step=0.01, format="%.6f", disabled=True)
    st.sidebar.number_input("blob_px", value=int(CFG.blob_px), step=1, disabled=True)
    st.sidebar.number_input("cut_max", value=int(CFG.cut_max), step=1, disabled=True)
    st.sidebar.number_input("cut_angle", value=float(CFG.cut_angle), step=0.5, format="%.6f", disabled=True)

    st.sidebar.number_input("pairing_angle_max", value=float(CFG.pairing_angle_max), step=0.5, format="%.6f", disabled=True)
    st.sidebar.number_input("pairing_length_for_calc_angle", value=int(CFG.pairing_length_for_calc_angle), step=1, disabled=True)

    st.sidebar.number_input("top_cut", value=int(CFG.top_cut), step=1, disabled=True)
    st.sidebar.number_input("post_eliminate_length_px", value=float(CFG.post_eliminate_length_px), step=0.5, format="%.6f", disabled=True)

    st.sidebar.number_input("hist_range_min_um", value=float(CFG.hist_range[0]), step=10.0, format="%.6f", disabled=True)
    st.sidebar.number_input("hist_range_max_um", value=float(CFG.hist_range[1]), step=10.0, format="%.6f", disabled=True)
    st.sidebar.number_input("hist_bins", value=int(CFG.hist_bins), step=1, disabled=True)
...
st.sidebar.write(f"threshold_otsu (recommended): {threshold_otsu:.6f}")
...
# ----------------------------
# Lower params + run button between upper and lower params
# ----------------------------
st.sidebar.markdown("---")
run_lower_button = st.sidebar.button("Run/Update (lower: skeletonize -> results)")
st.sidebar.markdown("---")
st.sidebar.header("Parameters (lower: skeletonize -> results)")

eliminate_length_px = int(st.sidebar.number_input("eliminate_length_px", value=int(CFG.eliminate_length_px), step=1))
border_margin_px = int(st.sidebar.number_input("border_margin_px", value=int(CFG.border_margin_px), step=1))
merge_short_seg_px = int(st.sidebar.number_input("merge_short_seg_px", value=int(CFG.merge_short_seg_px), step=1))

threshold_of_nonlinear = float(st.sidebar.number_input("threshold_of_nonlinear", value=float(CFG.threshold_of_nonlinear), step=0.01, format="%.6f"))
blob_px = int(st.sidebar.number_input("blob_px", value=int(CFG.blob_px), step=1))
cut_max = int(st.sidebar.number_input("cut_max", value=int(CFG.cut_max), step=1))
cut_angle = float(st.sidebar.number_input("cut_angle", value=float(CFG.cut_angle), step=0.5, format="%.6f"))

pairing_angle_max = float(st.sidebar.number_input("pairing_angle_max", value=float(CFG.pairing_angle_max), step=0.5, format="%.6f"))
pairing_length_for_calc_angle = int(st.sidebar.number_input("pairing_length_for_calc_angle", value=int(CFG.pairing_length_for_calc_angle), step=1))

top_cut = int(st.sidebar.number_input("top_cut", value=int(CFG.top_cut), step=1))
post_eliminate_length_px = float(st.sidebar.number_input("post_eliminate_length_px", value=float(CFG.post_eliminate_length_px), step=0.5, format="%.6f"))

hist_min_um = float(st.sidebar.number_input("hist_range_min_um", value=float(CFG.hist_range[0]), step=10.0, format="%.6f"))
hist_max_um = float(st.sidebar.number_input("hist_range_max_um", value=float(CFG.hist_range[1]), step=10.0, format="%.6f"))
hist_bins = int(st.sidebar.number_input("hist_bins", value=int(CFG.hist_bins), step=1))

# swap CFG for modules that reference it
cfg_mod.CFG = Config(
    um_per_px=float(um_per_px),
    background_is_dark=bool(background_is_dark),
    ...
    top_cut=int(top_cut),
    post_eliminate_length_px=float(post_eliminate_length_px),
    hist_range=(float(hist_min_um), float(hist_max_um)),
